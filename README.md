...
```bash
python -m pip install "git+ssh://git@github.com/WaVEV/mlflow-mongodb.git"
```

If SSH is not configured, use HTTPS instead:

```bash
python -m pip install "git+https://github.com/WaVEV/mlflow-mongodb.git"
```
...
```bash
git clone https://github.com/WaVEV/mlflow-mongodb.git
cd mlflow-mongodb
